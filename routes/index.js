...
exports.attendEvent = function(req, res) {
	Event.findOne({_id: req.params.eventId}, function(error, event) {
		if (event.attendees.length < config.maximumAttendees) {
			...
			paypalClient.setExpressCheckout({ 
				...
				, paymentrequest:[ {amt:'5.00', currencyCode: 'EUR', paymentAction: 'Sale'} ] 
...
exports.hangoutxml = function(req, res) {
	...
	res.send(fs.readFileSync('hangout.xml', encoding='utf8').replace('IFRAMEURL', config.baseUrl + '/hangout'));
};
